...
class Destroy2DEditor : Editor {
	...
	void OnSceneGUI() {
		...
		canDraw = !(((GameObject.Find( "SceneCamera" ).GetComponent<Camera>().pixelWidth - Event.current.mousePosition.x) < 80) &&
	...
	private Vector3 drawBrushGizmo() {
		...
		Vector2 mousePos = new Vector2(Event.current.mousePosition.x, GameObject.Find( "SceneCamera" ).GetComponent<Camera>().pixelHeight -  Event.current.mousePosition.y);
	...
	static void createDestroy2DObject() {
		...
		if (GameObject.Find("SceneCamera") != null) {
			Vector2 mousePos = new Vector2(GameObject.Find( "SceneCamera" ).GetComponent<Camera>().pixelWidth / 2,  GameObject.Find( "SceneCamera" ).GetComponent<Camera>().pixelHeight / 2);
